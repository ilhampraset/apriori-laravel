...
class Apriori {
    ...
    **/
    ...
    { 
       ...
       foreach($array as $item => $sup) 
          { 
             if($sup/count($db) * 10 >=$this->minSup)
                {
    ...
    **/
    ...
    {
        return round(($sup_ab / $sup_a) , 2);
    }
     private function expectedConfidence($sup_a, $total)
    {
        return round(($sup_a/$total) , 2) * 10;
    }
   
    private function liftRatio($confidence, $expectedCondfidence){
      return round(($confidence/$expectedCondfidence) , 2) ;
    }
    ...
    **/
                         ...
                         if($this->checkRule($subsets[$i], $subsets[$j]))
                            {
                               ...
                               $exC1 = $this->expectedConfidence($this->scan($subsets[$i]), count($db));
                               $exC2 = $this->expectedConfidence($this->scan($subsets[$j]), count($db));
                               if($c1>=$this->minConf)
                                  {
                                     $result[$n1]["then"] = $n2; 
                                     $result[$n1]["support"] = round(($scan/count($db)), 2) * 10;
                                     $result[$n1]["confidence"] = $c1;
                                     $result[$n1]["lift"] = $this->liftRatio($c1, $exC1);
                                  }
                                 
                               if($c2>=$this->minConf)
                                  { 
                                     $result[$n2]["then"] = $n1; 
                                     $result[$n2]["support"] = round(($scan/count($db)), 2) * 10;
                                     $result[$n2]["confidence"] = $c2;
                                     $result[$n2]["lift"] = $this->liftRatio($c2, $exC2);
                                  } 
    ...
    public function getFreqItemsets()
    {
       ...
       foreach($this->freqItmsts as $k => $v)
          {
             $tmp        = array();
             $tmp['sup'] = $this->allsups[$k]/count($this->dt) * 10 ;
             $k          = explode($this->delimiter, $k);
             $num        = count($k);
             for($i=0; $i<$num; $i++)
                {  
